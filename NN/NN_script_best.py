...
predictions_test = model.predict(X_v)
a=0
...
outs = ["Model number = "+str(file_num),"RMSE on training data "+str(rmse),"RMSE on validation data "+str(rmseValidate),"RMSE on test data "+str(rmseTest)]

outfile="NN/Saved/best/best_outputs.txt"
with open(outfile, "a+") as file:
    if "best_outputs.txt" not in best_saved:
        for line in first_outs:
            file.write(line)
            file.write("\n")
    for line in outs:
        file.write(line)
